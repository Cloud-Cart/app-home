...
export const FacebookMethod = (props: Props) => {
    ...
    const login = useCallback(() => {
        ...
        const popup = window.open(authUrl, "Facebook Login", `width=${width},height=${height},top=${top},left=${left}`);
        if (!popup) {
        ...
        const handleMessage = (event: MessageEvent<{ code: string, provider: string }>) => {
            ...
            facebookSocialLogin(event.data.code, REDIRECT_URI).then(response => {
    ...
    if (props.usage === 'button') {
        ...
        >
            {
                loading ?
                    <SpinnerLoader fill={'#316FF6'} width={'20px'} height={'20px'}/>
                    :
                    <>
            ...
            <div className={'bg-gray-700 rounded p-2'}>
                {
                    loading ?
                        ...
                        :
                        <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="-5 0 20 20"
                             version="1.1">
